...
export default function App() {
  ...
  const handleKeyDown = (event) => {
    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key.computerKey === event.key.toLowerCase()
          ? { ...key, active: true, keyPressed: true }
          : key
      )
    );

    const key = synthKeys.find((key) => key.computerKey === event.key.toLowerCase());
    if (key) {
  ...
  const handleMouseDown = (keyName) => {
    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key.keyName === keyName ? { ...key, active: true, keyPressed: true } : key
      )
    );

    const key = synthKeys.find((key) => key.keyName === keyName);
    if (key) {
      playNote(`./sounds/${currentInstrument}/${key.keyName}.mp3`);
    }
  };
  ...
  const handleMouseUp = (keyName) => {
    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key.keyName === keyName ? { ...key, active: false, keyPressed: false } : key
      )
    );
  };
